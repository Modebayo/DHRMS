...
const { getUserByEmail, getUserById, createAuthUser, getDocument, addDocument, setDocument } = require('./database');
...
function signin(req, res) {
    try {
        ...
        console.log('[signin] user lookup:', user ? user.uid : 'not found');
        if (!user) {
        ...
        let userDoc = getDocument('users', user.uid);
        let userData = null;
        if (userDoc) {
        ...
        setDocument('users', user.uid, userData, true);
...
function signup(req, res) {
    ...
    createAuthUser(uid, email.trim().toLowerCase(), password, userRole);
...
function changePassword(req, res) {
    try {
        ...
        updateAuthUser(user.uid, { password: newPassword });
